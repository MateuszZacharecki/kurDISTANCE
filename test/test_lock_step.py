...
def test_func_returns_zero_for_identical_arrays(func,
    ...
    if func.__name__ in ["inner_product", "harmonic_mean", "kumar_hassebrook", "cosine", "fidelity", "bhattacharyya"]:
...
def test_minkowski_1d_mathematical_equivalence(
    ...
    # Then
    assert minkowski_p1 == pytest.approx(manhattan, abs=1e-7)
    assert minkowski_p2 == pytest.approx(euclidean, abs=1e-7)
...
def test_minkowski_raises_type_error_when_too_few_arguments(
    ...
    # When / Then
    with pytest.raises(RuntimeError, match="Expected 3 arguments (array, array, p)"):
        ls.minkowski(x, y)
...
def test_minkowski_raises_type_error_for_non_1d_arrays() -> None:
    ...
    # When / Then
    with pytest.raises(TypeError, match="Expected 1D numpy arrays"):
        ls.minkowski(array_2d, array_1d)

    with pytest.raises(TypeError, match="Expected 1D numpy arrays"):
        ls.minkowski(array_1d, array_2d)

    with pytest.raises(TypeError, match="Expected 1D numpy arrays"):
        ls.minkowski(array_2d, array_2d)

    with pytest.raises(TypeError, match="Expected 1D numpy arrays"):
        ls.minkowski(array_0d, array_1d)

    with pytest.raises(TypeError, match="Expected 1D numpy arrays"):
        ls.minkowski(array_1d, array_0d)
...
def test_pairwise_minkowski_mathematical_equivalence(
    ...
    # Then
    assert_allclose(pairwise_minkowski_p1, pairwise_manhattan, atol=1e-7)
    assert_allclose(pairwise_minkowski_p2, pairwise_euclidean, atol=1e-7)
